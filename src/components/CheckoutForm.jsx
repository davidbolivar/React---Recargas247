...
const CheckoutForm = ({ handleSubmit, cart, orderStatus }) => {
	return (
		<>
			{cart.length > 0 && !orderStatus.completed && !orderStatus.error && (
				<main>
					<Cart />
			...
			{orderStatus.error && (
				<main className="container">
					<div className="px-4 py-5 my-5 text-center">
						<i className="bi bi-bag-x-fill text-danger display-1"></i>
						<h1 className="display-5 fw-bold">¡Opps! Ocurrió un error.</h1>
						<div className="col-lg-6 mx-auto">
							<p className="lead mb-4 text-secondary">{orderStatus.error}</p>
							{orderStatus.outOfStock.map((product, i) => (
								<p key={i} className="mb-4">
									<b>{i + 1}.-</b> {product.display_name.primary}
								</p>
							))}
							<div className="d-grid gap-2 d-sm-flex justify-content-sm-center">
								<Link to="/">
